...
import { ImageChangeResponseUser, NicknameChangeResponseUser, ResponseUser, UserNickname } from "src/type/user/user.type";



export function ChangePasswordDescription () {
  return applyDecorators(
    ApiResponse({ status: 200, description: 'if your client password was successfully changed.', type: ResponseUser}),
...
export function ChangeImageDescription() {
  return applyDecorators(
    ApiResponse({ status: 200, description: '프로필 변경이 성공적으로 완료되었을 때, (이미지경로) 반환 ', type: ImageChangeResponseUser}),
...
export function ChangeNicknameDescription() {
  return applyDecorators(
    ApiResponse({ status: 200, description: '프로필 변경이 성공적으로 완료되었을 때, (닉네임) 반환 ', type: NicknameChangeResponseUser}),
    ApiInternalServerErrorResponse({ description: '쿼리문 에러가 발생했을 때'}),
    ApiOperation({
      summary: 'change profile api',
      description: '이 api의 경우 유저의 프로필을 바꾸는 api입니다. 유저의 프로필은 닉네임과 프로필 사진으로 이루어져 있는데, 닉네임은 일반적인 폼데이터로 받을 수 있지만 사진의 경우 multipart/form-data 형식으로 보내야합니다.'
